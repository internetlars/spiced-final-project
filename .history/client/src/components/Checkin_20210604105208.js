...
export default class Checkin extends React.Component {
    ...
    checkInToCourt() {
        const newState = this.state.courtData.map((item) => {
            item.name === this.state.name ? { ...item, huhu: "hola" } : item;
            console.log("court with checkin", item);
            this.setState({ checkinSuccess: true });
        });

        this.setState({ courtData: newState });
